...
#define TEMPS_ANIMATION 1000 // 1 seconde pour toute l'animation
...
Personnage creerPersonnage(SDL_Renderer* rendu, int x, int y) {
    ...
    p.etat = ATTAQUE_1; // Commence directement par l'attaque 1
    p.indice_frame = 0;
    p.compteur_frame = 0;
...
void mettreAJourPersonnage(Personnage* p) {
    ...
    // Vérifier si le temps total d'animation est écoulé
...
void afficherPersonnage(SDL_Renderer* rendu, Personnage* p) {
    SDL_RenderCopy(rendu, p->attaques[0][p->indice_frame], NULL, &p->position);
}
...
int main(int argc, char* argv[]) {
    ...
    SDL_Window* fenetre = SDL_CreateWindow("Animation Attaque 1",
