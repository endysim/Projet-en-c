...
Personnage creerPersonnage(SDL_Renderer* rendu, int x, int y) {
    ...
    p.repos = chargerTexture(rendu, "naruto_sel.jpeg");

    char chemin[100];
    for (int attaque = 0; attaque < 3; ++attaque) {
        for (int i = 0; i < FRAMES_PAR_ATTAQUE; ++i) {
            snprintf(chemin, sizeof(chemin), "naruto_coup%d_%d.jpeg", attaque + 1, i);
...
int main(int argc, char* argv[]) {
    ...
    SDL_Window* fenetre = SDL_CreateWindow("Animations Naruto",
    ...
    // Lancer automatiquement l'attaque 3
