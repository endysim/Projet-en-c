...
Personnage creerPersonnage(SDL_Renderer* rendu, int x, int y) {
    ...
    p.etat = ATTAQUE_2; // Attaque 2 dès le début
    p.indice_frame = 0;
...
void afficherPersonnage(SDL_Renderer* rendu, Personnage* p) {
    SDL_RenderCopy(rendu, p->attaques[1][p->indice_frame], NULL, &p->position);
}
...
int main(int argc, char* argv[]) {
    ...
    SDL_Window* fenetre = SDL_CreateWindow("Animation Attaque 2",
