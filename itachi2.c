...
Personnage creerPersonnage(SDL_Renderer* rendu, int x, int y) {
    ...
    // Charger les images
    p.repos = chargerTexture(rendu, "itachi_sel.jpeg");

    char chemin[100];
    for (int attaque = 0; attaque < 3; ++attaque) {
        for (int i = 0; i < FRAMES_PAR_ATTAQUE; ++i) {
            snprintf(chemin, sizeof(chemin), "itachi_coup%d_%d.jpeg", attaque + 1, i);
...
int main(int argc, char* argv[]) {
    ...
    // Jouer automatiquement l'attaque 2 au démarrage
